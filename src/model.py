...
if 'x070' in os.environ["RWKV_MY_TESTING"]:
    ...
    flags = ["--save-temps", f'-D_C_={HEAD_SIZE}', f"-D_CHUNK_LEN_={CHUNK_LEN}","-O3"]
    ...
    if os.environ["RWKV_TRAIN_TYPE"] == 'states':
        wkv6state_cuda = load(name="wkv6state", sources=["cuda/wkv6state_op.cpp", f"cuda/wkv6state_cuda.cu"],
                        verbose=True, extra_cuda_cflags=["--save-temps", '-ffast-math', "-O3", f"-D_N_={HEAD_SIZE}", f"-D_T_={int(os.environ['RWKV_CTXLEN'])}"])
    ...
    wkv5_cuda = load(name="wkv5", sources=["cuda/wkv5_op.cpp", f"cuda/wkv5_cuda.cu"],
                    verbose=True, extra_cuda_cflags=["--save-temps", '-ffast-math', "-O3", f"-D_N_={HEAD_SIZE}"])
